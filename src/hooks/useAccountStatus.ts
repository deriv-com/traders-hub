...
export const useAccountStatus = () => {
    ...
    const modifiedAccountStatus = useMemo(() => {
        ...
        return {
            ...
            /** client tax information, place of birth and account opening reason is missing. */
            is_mt5_additional_kyc_required: status.has('mt5_additional_kyc_required'),
            /** the POI documents of the client will get expired soon, allow them to reupload POI documents. */
            is_poi_expiring_soon: status.has('poi_expiring_soon'),
            /** the POA documents of the client will get outdated soon, allow them to reupload POA documents. */
            is_poa_expiring_soon: status.has('poa_expiring_soon'),
            /** the client's tax_identification_number has been manually approved as client is not applicable for tax_identification_number */
            is_tin_manually_approved: status.has('tin_manually_approved'),
        };
    }, [data?.status]);

    return { data: modifiedAccountStatus, ...rest };
};
