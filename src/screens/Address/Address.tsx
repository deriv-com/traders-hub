...
import { useGetSettings, useStatesList } from '@deriv-com/api-hooks';
...
import { address } from '@/utils/validations';
...
export const Address = () => {
    const { data: getSettings } = useGetSettings();
    const country = getSettings?.country_code ?? '';
    const { data: statesList } = useStatesList({
        payload: {
            // @ts-expect-error The `states_list` type from `@deriv-com/api-hooks` is not correct.
